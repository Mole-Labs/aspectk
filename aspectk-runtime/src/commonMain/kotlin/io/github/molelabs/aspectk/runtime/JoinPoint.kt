...
public interface JoinPoint {
     ...
     * or `null` for top-level functions and functions called on companion objects.
     *
     * Example: for `fun UserService.getUser(id: String)` called as `userService.getUser("42")`,
     ...
     *
     * This object is allocated once per function definition (stored in a synthetic inner object
     * generated by the compiler plugin), not once per invocation.
     */
     ...
     *
     * Each element corresponds to the matching [MethodSignature.parameter] entry.
     * Elements may be `null` when the corresponding parameter type is nullable.
     *
     * **Type safety**: Cast elements to the expected type before use. For generic parameters,
     * the runtime type reflects type erasure — check [MethodParameter.typeName] for
     * the compile-time type name.
     */
